...
export async function getContactPageData() {
  ...
  try {
    const res = await fetch(`${WP_API_URL}/pages?slug=contact&_fields=id,title,acf&acf_format=standard`, { 
      cache: "no-store"
    });
    ...
    console.error("Failed to fetch contact page data from WP:", error);
    return null;
  }
}
